...
dat = dh['TKE_res'][int(100/20),:]
dat = dat/np.mean(dat[-dat.shape[0]/5:])
plt.plot(X/1000,dat,c=line_cols[0],label=R'$\mathregular{{N}_{xy1000}}$')
...
dh = loadmat('n_008_009.mat')
X = np.arange(dh['TKE_res'].shape[1])*dh['dx'][0][0]
dat = dh['TKE_res'][int(100/20),:]
dat = dat/np.mean(dat[-dat.shape[0]/5:])
plt.plot(X/1000,dat,c=line_cols[3],label=R'$\mathregular{{N}_{xy500+\theta}}$')

dh = loadmat('n_008_008.mat')
X = np.arange(dh['TKE_res'].shape[1])*dh['dx'][0][0]
dat = dh['TKE_res'][int(100/20),:]
dat = dat/np.mean(dat[-dat.shape[0]/5:])
plt.plot(X/1000,dat,c=line_cols[4],label=R'$\mathregular{{N}_{xy500+z750}}$')

dh = loadmat('n_008_010.mat')
X = np.arange(dh['TKE_res'].shape[1])*dh['dx'][0][0]
dat = dh['TKE_res'][int(100/20),:]
dat = dat/np.mean(dat[-dat.shape[0]/5:])
plt.plot(X/1000,dat,c=line_cols[8],label=R'$\mathregular{{N}_{z750+\theta}}$')

#dh = loadmat('n_008_012.mat')
#X = np.arange(dh['TKE_res'].shape[1])*dh['dx'][0][0]
#dat = dh['TKE_res'][int(100/20),:]
#dat = dat/np.mean(dat[-dat.shape[0]/5:])
#plt.plot(X/1000,dat,c=line_cols[8],label=R'$\mathregular{{N}_{z650+\theta}}$')
